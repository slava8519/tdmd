...
def _mk_zone(zid: int) -> ZoneRuntime:
    return ZoneRuntime(zid=zid, z0=0.0, z1=1.0, ztype=ZoneType.D, atom_ids=np.empty((0,), np.int32), n_cells=1)

def test_wfg_local_cycle_detect():
    zones = [_mk_zone(0), _mk_zone(1), _mk_zone(2)]
    cache = PersistentZoneLocalZBinsCache()
    autom = TDAutomaton1W(zones, box=10.0, cutoff=2.5, bins_cache=cache, traversal_order=[0,1,2], formal_core=True)
