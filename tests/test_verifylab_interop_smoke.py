...
def test_verifylab_interop_smoke():
    ...
    rows = sweep_verify_task("examples/interop/task.yaml", cfg.td,
                             steps=2, every=1,
                             zones_total_list=[2],
...
def test_verifylab_interop_smoke_binary_alloy_task():
    cfg = load_config("examples/td_1d_morse.yaml")
    rows = sweep_verify_task("examples/interop/task_alloy_pair.yaml", cfg.td,
                             steps=2, every=1,
                             zones_total_list=[2],
                             use_verlet_list=[False],
                             verlet_k_steps_list=[5],
...
def test_verifylab_interop_smoke_table_task():
    cfg = load_config("examples/td_1d_morse.yaml")
    rows = sweep_verify_task("examples/interop/task_table.yaml", cfg.td,
                             steps=2, every=1,
                             zones_total_list=[2],
                             use_verlet_list=[False],
                             verlet_k_steps_list=[5],
...
def test_verifylab_interop_eam_task_sync_mode():
    cfg = load_config("examples/td_1d_morse.yaml")
    rows = sweep_verify_task("examples/interop/task_eam_alloy.yaml", cfg.td,
                             steps=2, every=1,
                             zones_total_list=[2],
                             use_verlet_list=[False],
                             verlet_k_steps_list=[5],
