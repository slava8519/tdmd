...
def sweep_verify2(
    ...
    *,
    ...
    """
    if zones_total_list is None:
        zones_total_list = [int(cfg.td.zones_total)]
    if use_verlet_list is None:
        use_verlet_list = [bool(cfg.td.use_verlet)]
    if verlet_k_steps_list is None:
        verlet_k_steps_list = [int(cfg.td.verlet_k_steps)]
    if chaos_mode_list is None:
        chaos_mode_list = [bool(cfg.td.chaos_mode)]
    if chaos_delay_prob_list is None:
        chaos_delay_prob_list = [float(cfg.td.chaos_delay_prob)]
...
def sweep_verify_task(
    ...
    *,
    ...
    if zones_total_list is None:
        zones_total_list = [int(td_cfg.zones_total)]
    if use_verlet_list is None:
        use_verlet_list = [bool(td_cfg.use_verlet)]
    if verlet_k_steps_list is None:
        verlet_k_steps_list = [int(td_cfg.verlet_k_steps)]
    if chaos_mode_list is None:
        chaos_mode_list = [bool(td_cfg.chaos_mode)]
    if chaos_delay_prob_list is None:
        chaos_delay_prob_list = [float(td_cfg.chaos_delay_prob)]
