...
def _cell_index_z(pos: np.ndarray, box: float, rc: float, origin_z: float, ncell_z: int, wraps: bool):
    z = float(pos[2] % box)
    ...
    if iz < 0: iz = 0
    if iz >= ncell_z: iz = ncell_z - 1
    return iz

def _build_bins(r: np.ndarray, box: float, candidate_ids: np.ndarray, rc: float, z0: float, z1: float, step: int) -> ZoneUnwrappedZBins:
    rc = float(rc)
    ...
    return ZoneUnwrappedZBins(
        rc=rc, ncell_xy=ncell_xy, ncell_z=ncell_z, origin_z=origin_z, width_z=width_z, wraps=wraps,
        cell_atoms=cell_atoms, candidate_ids=candidate_ids.copy(), r_ref=r[candidate_ids].copy(),
...
class PersistentZoneLocalZBinsCache:
    ...
    def get(self, zid: int, r: np.ndarray, box: float, candidate_ids: np.ndarray,
            rc: float, skin_global: float, step: int, verlet_k_steps: int,
            z0: float, z1: float) -> ZoneUnwrappedZBins:
        zid = int(zid)
        candidate_ids = np.asarray(candidate_ids, dtype=np.int32)
        origin_z, width_z, wraps = _interval_unwrapped(z0, z1, box)
        ...
        if zc is None:
            ...
            if float(zc.rc) != float(rc):
                need_rebuild = True
            elif abs(zc.origin_z - origin_z) > 1e-12 or abs(zc.width_z - width_z) > 1e-9 or bool(zc.wraps) != bool(wraps):
                need_rebuild = True
            else:
...
def neighbor_candidates_for_atom(r_i: np.ndarray, box: float, zc: ZoneUnwrappedZBins):
    ...
    iz = int(np.floor(zz / rc))
    if iz < 0: iz = 0
    if iz >= zc.ncell_z: iz = zc.ncell_z-1
