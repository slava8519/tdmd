...
def _build_zone_cells(r: np.ndarray, box: float, candidate_ids: np.ndarray, rc: float) -> ZoneCells:
    ...
    return ZoneCells(rc=rc, ncell=ncell, cell_atoms=cell_atoms, idx_all=idx_all,
                     candidate_ids=candidate_ids.copy(), r_ref=r_ref, last_build_step=0)
...
class ZoneCellCache:
    ...
    def get(self, zid: int, r: np.ndarray, box: float, candidate_ids: np.ndarray,
            rc: float, skin_global: float, step: int, verlet_k_steps: int) -> ZoneCells:
        zid=int(zid)
        ...
        if zc is None:
            ...
            if float(zc.rc) != float(rc):
                ...
                if (step - zc.last_build_step) >= max(1,int(verlet_k_steps)):
                    ...
                    if candidate_ids.size != zc.candidate_ids.size or not np.array_equal(candidate_ids, zc.candidate_ids):
                        need=True
                    else:
                        if skin_global > 0.0 and _max_disp(r, box, candidate_ids, zc.r_ref) > 0.5*skin_global:
                            need=True
        if need:
