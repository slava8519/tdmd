...
def _build_neighbors(r: np.ndarray, box: float, candidate_ids: np.ndarray, rc: float) -> Dict[int, np.ndarray]:
...
class ZoneVerletCache:
    ...
    def get(self, zid: int, r: np.ndarray, box: float, candidate_ids: np.ndarray,
            rc: float, skin_global: float, step: int, verlet_k_steps: int) -> ZoneVerlet:
        ...
        if zv is None:
            ...
            if float(zv.rc) != float(rc):
                ...
                if (step - zv.last_build_step) >= max(1, int(verlet_k_steps)):
                    ...
                    if candidate_ids.size != zv.candidate_ids.size or not np.array_equal(candidate_ids, zv.candidate_ids):
                        need = True
                    else:
                        if skin_global > 0.0:
        ...
        if need:
            ...
            zv = ZoneVerlet(rc=float(rc), candidate_ids=candidate_ids.copy(), neighbors=neighbors,
                            r_ref=r_ref, last_build_step=int(step))
