...
def _run_verify_task_legacy(
    *,
    ...
    artifacts = _collect_verify_task_artifacts(
        ...
        observer_every=int(config.observer_every),
        decomposition=str(config.decomposition),
        zones_nx=int(config.zones_nx),
        zones_ny=int(config.zones_ny),
        zones_nz=int(config.zones_nz),
        sync_mode=bool(config.sync_mode),
        ...
        chaos_mode=bool(config.chaos_mode),
        chaos_seed=int(config.chaos_seed),
        chaos_delay_prob=float(config.chaos_delay_prob),
    )
    return _verify_task_result_from_artifacts(
        artifacts=artifacts,
        case_name=str(config.case_name),
        steps=int(config.steps),
        tol_dr=float(config.tol_dr),
        tol_dv=float(config.tol_dv),
        tol_dE=float(config.tol_dE),
        tol_dT=float(config.tol_dT),
        tol_dP=float(config.tol_dP),
    )
