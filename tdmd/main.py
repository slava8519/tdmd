...
from .io import load_task, export_lammps_data, export_lammps_in, task_to_arrays, validate_task_for_run
...
def main():
    ...
    args=p.parse_args()
    cfg = None
    if args.cmd != "run" or (args.cmd == "run" and args.config):
        cfg = load_config(args.config)


    if args.cmd == "golden-gen":
        if cfg is None:
            raise SystemExit("config required for golden-gen")
        pot=make_potential(cfg.potential.kind, cfg.potential.params)
        from .verify_golden import generate_golden
        out = generate_golden(cfg=cfg, potential=pot, out_path=args.out, steps=int(args.steps), every=int(args.every))
        print(f"[golden-gen] wrote {out}")
        raise SystemExit(0)

    if args.cmd == "golden-check":
        if cfg is None:
            raise SystemExit("config required for golden-check")
        pot=make_potential(cfg.potential.kind, cfg.potential.params)
        from .verify_golden import check_against_golden
        res = check_against_golden(cfg=cfg, potential=pot, golden_path=args.golden, steps=int(args.steps), every=int(args.every),
                                  tol_dE=float(args.tol_dE), tol_dT=float(args.tol_dT), tol_dP=float(args.tol_dP))
        ok=True
        for r in res:
            print(f"[golden-check {r.case}] ok={r.ok} max|dE|={r.max_dE:.3e} max|dT|={r.max_dT:.3e} max|dP|={r.max_dP:.3e}")
            ok = ok and r.ok
        raise SystemExit(0 if ok else 2)

    if args.cmd == "verifylab":
        if cfg is None:
            raise SystemExit("config required for verifylab")
        pot=make_potential(cfg.potential.kind, cfg.potential.params)
        from .verify_lab import sweep_verify2, write_csv, summarize
        ...
        rows = sweep_verify2(
            cfg, pot,
            steps=max(1,int(args.steps)), every=max(1,int(args.every)),
        ...
        summ = summarize(rows)
        print(f"[verifylab] wrote {args.csv}  total={summ['total']} ok={summ['ok']} fail={summ['fail']}")
        for c, s in summ["by_case"].items():
            print(f"  - {c}: ok {s['ok']}/{s['total']}  worst dE={s['worst']['max_dE']:.3e} dP={s['worst']['max_dP']:.3e}")
        if args.plots:
    ...
    if args.cmd == "verify2":
        if cfg is None:
            raise SystemExit("config required for verify2")
        pot=make_potential(cfg.potential.kind, cfg.potential.params)
        ...
        if args.case:
            ...
            if not cases:
                raise SystemExit(f"Unknown cases: {args.case}. Known: {[c.name for c in default_cases()]}")
        res = run_verify_v2(
            potential=pot, mass=cfg.system.mass, dt=cfg.run.dt, cutoff=cfg.run.cutoff,
            cell_size=cfg.td.cell_size, zones_total=cfg.td.zones_total,
            ...
            use_verlet=cfg.td.use_verlet, verlet_k_steps=cfg.td.verlet_k_steps,
            steps=max(1,int(args.steps)), observer_every=max(1,int(args.every)),
            tol_dr=float(args.tol_dr), tol_dv=float(args.tol_dv),
            tol_dE=float(args.tol_dE), tol_dT=float(args.tol_dT), tol_dP=float(args.tol_dP),
        ...
        ok = True
        for r in res:
            print(f"[verify2 {r.case}] ok={r.ok}  max|dr|={r.max_dr:.3e} max|dv|={r.max_dv:.3e}  "
                  f"max|dE|={r.max_dE:.3e} max|dT|={r.max_dT:.3e} max|dP|={r.max_dP:.3e}", flush=True)
            ok = ok and r.ok
        raise SystemExit(0 if ok else 2)
    ...
    if args.cmd == "run":
        ...
        if task is not None:
            ...
            allowed_ensembles = ("nve", "nvt", "npt") if args.mode == "serial" else ("nve", "nvt", "npt")
        ...
        if args.mode == "serial":
            output = OutputBundle(
                traj=TrajectoryWriter(
                    ...
                    compression=str(args.traj_compression),
                    write_output_manifest=output_manifest,
                    force_potential=pot,
                    force_cutoff=float(cutoff),
                    force_atom_types=atom_types,
                ) if args.traj else None,
                metrics=MetricsWriter(
                    args.metrics, mass=mass, box=box, cutoff=cutoff, potential=pot, atom_types=atom_types,
                    write_output_manifest=output_manifest,
                ) if args.metrics else None,
            )
            def obs(step, r, v, box_cur=None):
                box_now = float(box if box_cur is None else box_cur)
            ...
            run_serial(r0, v0, mass, box, pot, dt, cutoff, n_steps,
                       thermo_every=thermo_every,
                       observer=obs if obs_every else None, observer_every=obs_every,
                       atom_types=atom_types,
        ...
        if args.mode == "td_local":
            ...
            run_td_local(
                r0, v0, mass, box, pot,
                dt=dt, cutoff=cutoff, n_steps=n_steps,
                observer=obs if obs_every else None, observer_every=obs_every,
                atom_types=atom_types,
                cell_size=cfg.td.cell_size, zones_total=cfg.td.zones_total,
                zone_cells_w=cfg.td.zone_cells_w, zone_cells_s=cfg.td.zone_cells_s,
        ...
        run_td_full_mpi_1d(
            ...
            zones_nx=cfg.td.zones_nx, zones_ny=cfg.td.zones_ny, zones_nz=cfg.td.zones_nz,
    ...
    # verify
    if cfg is None:
        raise SystemExit("config required for verify")
    pot=make_potential(cfg.potential.kind, cfg.potential.params)
    r0=init_positions(cfg.system.n_atoms, cfg.system.box, cfg.system.seed)
    v0=init_velocities(cfg.system.n_atoms, cfg.system.temperature, cfg.system.mass, cfg.system.seed)
    ...
    run_serial(rA, vA, cfg.system.mass, cfg.system.box, pot,
               cfg.run.dt, cfg.run.cutoff, steps, thermo_every=0,
               atom_types=None,
               ensemble_kind=str(cfg.ensemble.kind),
               thermostat=cfg.ensemble.thermostat,
               barostat=cfg.ensemble.barostat)

    run_td_local(rB, vB, cfg.system.mass, cfg.system.box, pot,
             dt=cfg.run.dt, cutoff=cfg.run.cutoff, n_steps=steps,
             cell_size=cfg.td.cell_size, zones_total=cfg.td.zones_total,
             zone_cells_w=cfg.td.zone_cells_w, zone_cells_s=cfg.td.zone_cells_s,
             zone_cells_pattern=cfg.td.zone_cells_pattern, traversal=cfg.td.traversal,
             buffer_k=cfg.td.buffer_k, skin_from_buffer=cfg.td.skin_from_buffer,
             use_verlet=cfg.td.use_verlet, verlet_k_steps=cfg.td.verlet_k_steps,
             decomposition=str(getattr(cfg.td,'decomposition','1d')),
             zones_nx=int(getattr(cfg.td,'zones_nx',1)), zones_ny=int(getattr(cfg.td,'zones_ny',1)), zones_nz=int(getattr(cfg.td,'zones_nz',1)),
             strict_min_zone_width=cfg.td.strict_min_zone_width,
             ensemble_kind=str(cfg.ensemble.kind),
             thermostat=cfg.ensemble.thermostat,
             barostat=cfg.ensemble.barostat)
    ...
    print(f"[verify] steps={steps} max|dr|={float(dr.max()):.6e}  max|dv|={float(dv.max()):.6e}", flush=True)
