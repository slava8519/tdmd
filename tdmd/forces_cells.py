...
def forces_on_targets_zonecells(r: np.ndarray, box: float, potential, cutoff: float,
                                target_ids: np.ndarray, zc,
...
def forces_on_targets_celllist(r: np.ndarray, box: float, potential, cutoff: float,
                               target_ids: np.ndarray, cell,
                               atom_types: np.ndarray | None = None) -> np.ndarray:
...
def forces_on_targets_celllist_compact(r: np.ndarray, box: float, potential, cutoff: float,
                                       target_ids: np.ndarray, cell,
                                       atom_types: np.ndarray | None = None) -> np.ndarray:
    """Forces on target_ids using CellList; returns array (len(target_ids),3)."""
    f_full = forces_on_targets_celllist(r, box, potential, cutoff, target_ids, cell, atom_types=atom_types)
