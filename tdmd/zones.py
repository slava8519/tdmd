...
class Zone3D:
    ...
    def contains(self, r: np.ndarray, box: float) -> np.ndarray:
        rr = r % box
        return ((rr[:,0] >= self.lo[0]) & (rr[:,0] < self.hi[0]) &
                (rr[:,1] >= self.lo[1]) & (rr[:,1] < self.hi[1]) &
                (rr[:,2] >= self.lo[2]) & (rr[:,2] < self.hi[2]))
...
def zones_overlapping_aabb_pbc(lo: np.ndarray, hi: np.ndarray, box: float, layout: ZoneLayout3DBlocks) -> List[int]:
