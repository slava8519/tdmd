...
def _run(cmd: list[str], cwd: Path, timeout: int, env: dict[str, str]) -> tuple[int, float, str, str]:
    t0 = time.perf_counter()
    proc = subprocess.run(cmd, cwd=str(cwd), timeout=int(timeout), capture_output=True, text=True, env=env)
    return int(proc.returncode), float(time.perf_counter() - t0), str(proc.stdout or ""), str(proc.stderr or "")
...
def _iter_rows(
    *,
    ...
    for overlap in overlap_modes:
        ...
        if bool(simulate):
            elapsed = _sim_elapsed(n_ranks=int(n_ranks), overlap=overlap_i, cuda_aware=bool(enable_cuda_aware_mpi))
            rows.append(
                {
                    "overlap": overlap_i,
        ...
        try:
            ...
            for _ in range(1, attempts + 1):
                rc_try, elapsed_try, out_try, err_try = _run(cmd, cwd=root, timeout=int(timeout), env=run_env)
                rc = int(rc_try)
...
def _write_outputs(*, args, rows: list[dict[str, int | float]], cfg_path: Path, run_summary: dict) -> None:
    ...
    with open(args.out, "w", newline="", encoding="utf-8") as f:
        ...
        t_base = next((float(r["elapsed_sec"]) for r in rows if int(r["overlap"]) == 0 and int(r["returncode"]) == 0), None)
        for r in rows:
            t = float(r["elapsed_sec"])
            sp = (t_base / t) if (t_base is not None and t > 0 and int(r["returncode"]) == 0) else 0.0
            w.writerow(
                [
                    int(r["overlap"]),
...
def main() -> int:
    ...
    p.add_argument("--overlap-list", default="", help="Comma-separated overlap modes (0=blocking,1=isend)")
    p.add_argument("--cuda-aware", action="store_true", help="Enable td.cuda_aware_mpi for overlap=1 runs")
    ...
    p.add_argument("--thermo-every", type=int, default=0, help="Override run.thermo_every in generated configs")
    p.add_argument("--fabric", default="", help="Free-text transport/fabric label for artifact normalization")
    p.add_argument("--simulate", action="store_true", help="Force simulated execution (no MPI launch)")
    p.add_argument(
    ...
    cfg_rel = str(args.config).strip() or (
        str(profile.runtime.config) if profile is not None else "examples/td_1d_morse_static_rr_smoke4.yaml"
    )
    ...
    n_ranks = int(args.n) if int(args.n) > 0 else (
        int(profile.runtime.ranks_default[0]) if profile is not None else 4
    )
    timeout = int(args.timeout) if int(args.timeout) > 0 else (
        int(profile.runtime.timeout_sec) if profile is not None else 180
    )
    retries = max(1, int(args.retries)) if int(args.retries) > 0 else (
        int(profile.runtime.retries) if profile is not None else 2
    )
    n_steps = int(args.steps) if int(args.steps) > 0 else (
        int(profile.runtime.stability_steps) if profile is not None else 0
    )
    thermo_every = int(args.thermo_every) if int(args.thermo_every) > 0 else (
        int(profile.runtime.stability_thermo_every) if profile is not None else 1
    )
    ...
    run_env = apply_profile_env(dict(os.environ), profile.runtime.env if profile is not None else {})

    mpirun = _find_mpirun(str(args.mpirun).strip() or (profile.runtime.mpirun if profile is not None else ""))
    if not simulate and not mpirun:
        if allow_simulated:
            simulate = True
        else:
            print("[bench-mpi-overlap] mpirun/mpiexec not found and simulated fallback is disabled", file=sys.stderr)
            return 2

    if args.dry_run:
        print(f"[bench-mpi-overlap] dry-run config={cfg_path}")
        print(f"[bench-mpi-overlap] ranks={int(n_ranks)} overlap_modes={overlap_modes} cuda_aware={bool(cuda_aware)}")
        print(f"[bench-mpi-overlap] mpirun={'<simulated>' if simulate else ('<auto>' if not args.mpirun else args.mpirun)}")
        return 0
